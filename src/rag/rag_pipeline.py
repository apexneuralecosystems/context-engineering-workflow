...
from src.rag.retriever import QdrantVectorDB
    ...
    def retrieve_context(self, query: str, top_k: int = 3) -> List[Dict[str, Any]]:
        try:
            ...
            if not self.embeddings.api_key:
                raise Exception("VOYAGE_API_KEY is not set. Cannot generate embeddings for query.")
            
            # Generate query embedding
            query_embedding = self.embeddings.embed_query(query)
            if not query_embedding:
                raise Exception("Failed to generate query embedding. Check VOYAGE_API_KEY and API connectivity.")
            
            # Check if collection exists and has data
            doc_count = self.vector_db.get_collection_count()
            if doc_count == 0:
                raise Exception("No documents in vector database. Upload a document first.")
            ...
            if not search_results:
                # Return empty list instead of raising - this is a valid case
                return []
            ...
            if "voyage" in error_str or "api_key" in error_str:
                raise Exception(f"Embedding API error: {str(e)}. Ensure VOYAGE_API_KEY is set correctly.") from e
            elif "no documents" in error_str or "collection" in error_str:
                raise Exception(f"Vector database error: {str(e)}") from e
            else:
                raise Exception(f"RAG retrieval failed: {str(e)}") from e
