...
class RAGTool(BaseTool):
    ...
    def _run(self, query: str, top_k: int = 3, document_paths: List[str] = None):
        try:
            # Check document count with better error handling
            try:
                ...
                if "not found" in error_str or "does not exist" in error_str:
                    error_msg = f"Vector database collection does not exist. Upload a document first to create the collection."
                elif "lock" in error_str or "already accessed" in error_str:
                    error_msg = f"Database is locked by another process. Please restart the application."
                else:
                    error_msg = f"Failed to check vector database: {str(count_error)}"
            ...
            try:
                ...
                # Provide more specific error messages
                if "voyage" in error_str or "api_key" in error_str or "embedding" in error_str:
                    error_msg = f"Embedding API error: {str(retrieve_error)}. Check VOYAGE_API_KEY is set correctly and the API is accessible."
                elif "no documents" in error_str or "collection" in error_str or "not found" in error_str:
                    error_msg = f"Vector database error: {str(retrieve_error)}. Ensure documents have been uploaded and processed."
                elif "lock" in error_str or "already accessed" in error_str:
                    error_msg = f"Database locked: {str(retrieve_error)}. Try restarting the backend server."
                else:
                    error_msg = f"RAG retrieval failed: {str(retrieve_error)}"
            ...
            if "qdrant" in error_str or "storage" in error_str or "lock" in error_str:
                error_details["answer"] = f"RAG search failed: Database access error. {str(e)}\n\nThis may occur if the database is locked by another process. Try restarting the application."
            elif "embedding" in error_str:
                error_details["answer"] = f"RAG search failed: Embedding generation error. {str(e)}\n\nCheck that VOYAGE_API_KEY is set correctly."
            elif "collection" in error_str or "not found" in error_str:
                error_details["answer"] = f"RAG search failed: Vector database collection error. {str(e)}\n\nThe collection may not exist. Try uploading a document first."
