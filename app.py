...
class StreamlitResearchAssistant:
    ...
    def initialize(self) -> bool:
        try:
            # Check if Qdrant is already initialized to avoid lock conflicts
            if st.session_state.get('qdrant_initialized', False):
                # Try to reuse existing assistant if available
                if st.session_state.get('assistant') and st.session_state.assistant.initialized:
                    self.flow = st.session_state.assistant.flow
            ...
            error_msg = str(e)
            if "already accessed" in error_msg or "lock" in error_msg.lower():
                st.error(f"⚠️ Qdrant database is locked. Please restart the Streamlit app completely (stop and start again).\n\nError: {error_msg}")
            else:
                st.error(f"Failed to initialize Research Assistant: {error_msg}")
            return False
...
def create_research_assistant() -> Optional[StreamlitResearchAssistant]:
    # Check if assistant already exists in session state
    if 'assistant' in st.session_state and st.session_state.assistant is not None:
        if st.session_state.assistant.initialized:
    ...
    try:
        ...
        error_msg = str(e)
        if "already accessed" in error_msg or "lock" in error_msg.lower():
            st.error(f"⚠️ Qdrant database is locked. Please:\n1. Stop the Streamlit app completely\n2. Wait a few seconds\n3. Start it again\n\nError: {error_msg}")
        else:
            st.error(f"Failed to create Research Assistant: {error_msg}")
        return None
    except Exception as e:
        ...
        with st.expander(title, expanded=False):
            ...
            for source_name, source_data, icon in sources:
                ...
                if source_name == 'Memory (History)':
                    ...
                    # Parse RAG status - it might be a JSON string
                    raw_status = source_data.get('status', 'UNKNOWN')
                    if isinstance(raw_status, str):
                        try:
                            import json
                            parsed = json.loads(source_data.get('answer', '{}'))
                            status = parsed.get('status', raw_status)
                        except:
                            status = raw_status
                    else:
                        status = raw_status
                ...
                with st.expander(f"{icon} **{source_name}** ({status})", expanded=False):
                    ...
                    if status == 'OK':
                        if source_name == 'Memory (History)':
                            ...
                            if status == 'ERROR':
                                error_msg = source_data.get('error', source_data.get('answer', 'Unknown error'))
                                st.error(f"**RAG Error:** {error_msg}")
                                ...
                                doc_count = retrieval_metadata.get('document_count', 0)
                                if doc_count == 0:
                                    st.info("💡 **Tip:** No documents found in the vector database. Upload a document first to enable RAG search.")
                                else:
                                    st.info(f"Database contains {doc_count} document(s), but retrieval failed.")
                            elif status == 'INSUFFICIENT_CONTEXT':
                                answer = source_data.get('answer', 'No context available')
                                st.warning(f"**Insufficient Context:** {answer}")
                                
                                retrieval_metadata = source_data.get('retrieval_metadata', {})
                                doc_count = retrieval_metadata.get('document_count', 0)
                                if doc_count == 0:
                                    st.info("💡 **Tip:** Upload a document to add context to the RAG system.")
                            elif status == 'OK':
                                # Parse answer if it's JSON
                                ...
                                if isinstance(answer, str):
                                    ...
                                    ellipsis = '...' if len(answer) > 500 else ''
                                    st.markdown(f"{preview}{ellipsis}")
                                else:
                                ...
                                if citations:
                                    ...
                                    for citation in citations:
                                        if isinstance(citation, dict):
                                            ...
                                            st.markdown(f"• **{label}** (Page {page}, Chunk {chunk}, Score: {score:.3f})")
                                        else:
                                            st.markdown(f"• {str(citation)}")
                            else:
                                st.markdown("**Content:**")
                                answer = source_data.get('answer', 'No answer available')
                                st.markdown(str(answer)[:500])
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            ...
            <div style="display: flex; justify-content: center; align-items: center; gap: 8px; margin-bottom: 20px;">
                ...
                <div style="display: flex; align-items: center; gap: 25px; margin-left: 15px;">
                    ...
                    <a href="https://qdrant.tech/" style="display: inline-block; vertical-align: middle;">
                        <img src="https://cdn.jsdelivr.net/gh/qdrant/qdrant@master/docs/img/qdrant-logo-full.svg" 
                             alt="Qdrant" style="height: 28px;">
                    </a>
                </div>
            </div>
