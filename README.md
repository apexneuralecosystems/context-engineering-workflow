...
2. **Set up environment variables:**
   ...
   # OR use OpenRouter instead (supports multiple LLM providers)
   ...
   - [OpenAI →](https://openai.com) or [OpenRouter →](https://openrouter.ai/) (alternative)
...
3. **Set up Qdrant Vector Database (Embedded Mode):**
   
   Qdrant runs in embedded mode with local storage - **no Docker or server setup required!**
   
   The vector database will be stored locally in the `./qdrant_db` directory by default.
   
   **Optional:** You can customize the storage path by adding to your `.env` file:
   ```env
   QDRANT_DB_PATH=./qdrant_db
   ```
   
   That's it! Qdrant will automatically create and manage the database files when you first use it.
